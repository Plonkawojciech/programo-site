...
function ProjectImage({ src, alt, priority = false, parallaxY = 0, accentColor }: { src: string, alt: string, priority?: boolean, parallaxY?: any, accentColor?: string }) {
  ...
  if (isPlaceholder) {
    ...
    return (
      <motion.div style={parallaxY ? { y: parallaxY } : {}} className="absolute inset-0 flex items-center justify-center bg-dark-card border border-border">
        <span className="text-text-muted/40 font-headline text-2xl uppercase tracking-widest font-bold text-center px-6">
          [Wstaw zdjecie: {placeholderText}]
        </span>
      </motion.div>
    );
  }
  ...
  if (isIcon) {
    ...
    return (
      <motion.div style={parallaxY ? { y: parallaxY } : {}} className="absolute inset-0 flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at center, ${accent}18 0%, #050505 70%)` }} />
...
function ProjectContent({ slug }: { slug: string }) {
  ...
  return (
    <div className="min-h-screen bg-dark selection:bg-coral/20 selection:text-coral">
      <Navbar />

      <main>
        {/* Cinematic Hero */}
        <section ref={headerRef} className="relative h-screen w-full overflow-hidden bg-dark">
          <motion.div style={{ scale: imageScale }} className="absolute inset-0 h-full w-full">
            {project.subProducts && project.subProducts.length > 0 ? (
              <motion.div style={headerY ? { y: headerY } : {}} className="absolute inset-0">
                <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at 50% 40%, ${project.accentColor}15 0%, #050505 65%)` }} />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex items-center gap-8 md:gap-16 2xl:gap-24">
                    {project.subProducts.map((sub: SubProduct, i: number) => (
                      <motion.div
                        key={sub.name}
          ...
          <div className="relative flex h-full flex-col justify-end px-6 pb-24 md:px-24 2xl:px-40 text-text mx-auto max-w-[2560px]">
            <motion.div
            ...
            >
              <div className="flex items-center gap-6 mb-8 overflow-hidden">
                <motion.span initial={{ x: -50 }} animate={{ x: 0 }} transition={{ delay: 0.8, duration: 1 }} className="text-[10px] md:text-xs 2xl:text-sm font-bold uppercase tracking-[0.5em] text-coral">
                  {project.year}
                </motion.span>
                <motion.span initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 1, duration: 1 }} className="w-16 h-px bg-coral/50 origin-left" />
                <motion.span initial={{ x: 50 }} animate={{ x: 0 }} transition={{ delay: 0.8, duration: 1 }} className="text-[10px] md:text-xs 2xl:text-sm font-bold uppercase tracking-[0.5em] text-text/70">
                  {project.tags[0]}
                </motion.span>
              </div>
        ...
        {/* The Story / Overview */}
        <section className="relative z-10 bg-dark-surface px-6 py-32 md:px-24 2xl:px-40 md:py-48">
          <div className="mx-auto max-w-[2560px]">
            ...
            >
              <div className="lg:col-span-8 flex flex-col gap-12">
                <h2 className="font-headline text-5xl md:text-7xl 2xl:text-8xl font-medium tracking-tight text-text leading-[1.1] mb-12">
                ...
                <div className="flex flex-col gap-10">
                  {formattedDescription.map((paragraph, idx) => {
                    ...
                    return (
                      <p key={idx} className="text-xl md:text-2xl 2xl:text-4xl font-light leading-relaxed text-text-muted max-w-4xl">
                        {paragraph}
                      </p>
                    );
                  })}
              ...
              <div className="lg:col-span-4 flex flex-col gap-16 lg:pl-16 lg:border-l border-white/10">
                <div>
                  <span className="block w-4 h-px bg-coral mb-6" />
                  <h3 className="text-[10px] 2xl:text-xs font-bold uppercase tracking-widest text-text-muted mb-4">Role</h3>
                  <p className="font-headline text-3xl 2xl:text-4xl text-text">{project.role[lang]}</p>
                </div>

                <div>
                  <span className="block w-4 h-px bg-coral mb-6" />
                  <h3 className="text-[10px] 2xl:text-xs font-bold uppercase tracking-widest text-text-muted mb-6">Technologies</h3>
                  <div className="flex flex-col gap-3">
                    {project.tech.map((t) => (
                      <span key={t} className="text-base 2xl:text-xl font-light text-text border-b border-white/5 pb-3">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                {project.liveUrl && (
                  <div className="pt-8">
                    <a href={project.liveUrl} target="_blank" className="group inline-flex items-center gap-6">
                      <span className="relative flex h-16 w-16 items-center justify-center rounded-full bg-coral text-dark transition-transform duration-500 group-hover:scale-110">
                      ...
                      <span className="text-sm 2xl:text-lg font-bold uppercase tracking-[0.3em] text-text group-hover:text-coral transition-colors">
                        Launch Experience
                      </span>
                    </a>
                  </div>
        ...
        {project.subProducts && project.subProducts.length > 0 && (
          <>
            <section className="bg-dark py-24 md:py-40 px-6 md:px-24 2xl:px-40 overflow-hidden">
              <div className="mx-auto max-w-[2560px]">
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.5em] text-text/30 block mb-8">
                    {lang === "pl" ? "Ekosystem" : "Ecosystem"}
                  </span>
                  <h2 className="font-headline text-5xl md:text-8xl 2xl:text-[9vw] font-bold tracking-tighter text-text leading-[0.9]">
            ...
            {project.subProducts.map((sub: SubProduct, idx: number) => {
              const isReversed = idx % 2 !== 0;
              const accent = sub.accentColor || project.accentColor;

              return (
                <section key={sub.name} className="bg-dark overflow-hidden px-6 md:px-24 2xl:px-40 py-20 md:py-28">
                  <div className="mx-auto max-w-[2560px] grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center">
                    ...
                    >
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.4em] text-text/30">
                          {(idx + 1).toString().padStart(2, '0')} / {project.subProducts!.length.toString().padStart(2, '0')}
                        </span>
                      </div>
                      <h3 className="font-headline text-4xl md:text-6xl font-bold tracking-tight text-text mb-3 leading-[1.05]">
                        {sub.name}
                      </h3>
                      <p className="font-headline text-xl md:text-2xl font-light mb-6" style={{ color: accent }}>
                        {sub.tagline[lang]}
                      </p>
                      <p className="text-base md:text-lg font-light leading-relaxed text-text-muted mb-8 max-w-lg">
                      ...
                      {sub.liveUrl && (
                        <a href={sub.liveUrl} target="_blank" className="group inline-flex items-center gap-4">
                          ...
                          <span className="text-xs font-bold uppercase tracking-[0.3em] text-text/50 group-hover:text-text transition-colors">
                  ...
                  {idx < project.subProducts!.length - 1 && (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
                      className="mx-auto max-w-[2560px] h-px bg-white/5 origin-left mt-20 md:mt-28"
                    />
                  )}
                </section>
              );
        ...
        {/* Capabilities */}
        <section className="bg-dark-surface py-24 md:py-32 px-6 md:px-24 2xl:px-40 border-t border-white/5">
          <div className="mx-auto max-w-[2560px]">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className="font-headline text-5xl md:text-8xl 2xl:text-[8vw] font-bold tracking-tighter text-text">
                Platform <span className="italic text-coral font-light">Capabilities.</span>
              </h2>
            </motion.div>
              ...
              {project.features[lang].map((f, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex flex-col gap-6 md:gap-10 border-t border-white/10 pt-8"
                  >
                    <span className="font-headline text-2xl italic text-coral/40">{(i + 1).toString().padStart(2, '0')}</span>
                    {hasTitle ? (
                      <div>
                        <h4 className="font-headline text-3xl 2xl:text-5xl font-medium text-text mb-4 leading-tight">{parts[0].trim()}</h4>
                        <p className="text-lg 2xl:text-2xl font-light leading-relaxed text-text-muted">
                          {parts[1].trim()}
                        </p>
                      </div>
                    ) : (
                      <h4 className="font-headline text-3xl 2xl:text-5xl font-medium text-text leading-tight">
                        {cleanF}
                      </h4>
                    )}
                  </motion.div>
        ...
        {project.screenshots && project.screenshots.length > 2 && !hasIconScreenshots && !project.subProducts?.length && (
          <section className="w-full pb-32 md:pb-48 px-6 md:px-24 2xl:px-40 bg-dark">
            ...
            <Link href={`/projects/${next.slug}`} className="group flex h-full w-full items-center justify-center">
              ...
              <div className="relative z-10 text-center flex flex-col items-center">
                <motion.div
                  ...
                  transition={{ duration: 1 }}
                >
                  <span className="mb-8 block text-[10px] md:text-xs 2xl:text-sm font-bold uppercase tracking-[0.5em] text-text/50 group-hover:text-coral transition-colors">
                    Next Project
                  </span>
                  <h2 className="font-headline text-[15vw] font-bold leading-none tracking-tighter text-text md:text-[10vw]">
                    {next.title}
                  </h2>
                </motion.div>
              </div>
