...
function FadeInText({ text }: { text: string }) {
  ...
  return (
    <motion.p
      ref={container}
      style={{ opacity, y }}
      className="mb-12 text-3xl font-light leading-tight text-[#1A1816]/80 md:text-5xl lg:text-6xl"
    >
      {text}
    </motion.p>
  );
...
export default function About() {
  ...
  return (
    <section id="about" className="relative overflow-hidden bg-[#FAF8F4] py-24 md:py-32 lg:py-56 rounded-t-[32px] 2xl:rounded-t-[64px] rounded-b-[32px] 2xl:rounded-b-[64px]">
      ...
      <div className="mx-auto max-w-[2560px] px-6 md:px-24 2xl:px-40">
        ...
        <div className="mt-24 md:mt-32 2xl:mt-48 grid grid-cols-2 gap-x-8 gap-y-16 md:gap-20 md:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            >
              ...
              <span className="font-headline text-4xl md:text-5xl 2xl:text-[5vw] font-bold italic tracking-tighter text-[#1A1816]">
                {stat.value}
              </span>
            </motion.div>
          ))}
        ...
        <div className="mt-32 md:mt-48 py-20 border-y border-[#1A1816]/10 relative">
          <motion.div
             initial={{ opacity: 0, scale: 0.9 }}
             whileInView={{ opacity: 1, scale: 1 }}
             viewport={{ once: true }}
             transition={{ duration: 0.8 }}
             className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[30vw] font-headline font-black text-[#1A1816]/5 pointer-events-none select-none"
          >
            FUTURE
          </motion.div>
