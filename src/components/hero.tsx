...
function AIScannerCell() {
  ...
  const handleScannerMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const hLine = el.querySelector("[data-scanner-h]") as HTMLElement;
    const vLine = el.querySelector("[data-scanner-v]") as HTMLElement;
    if (hLine) hLine.style.transform = `translateY(${y}px)`;
    if (vLine) vLine.style.transform = `translateX(${x}px)`;
  };
        ...
        <div className="w-full h-full grid grid-cols-2 md:grid-cols-4 grid-rows-4 md:grid-rows-4 gap-[1px]">
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
              <div className="absolute inset-0 animate-shimmer-sweep opacity-10" />
            </div>
          </motion.div>
