...
export const projects: Project[] = [
  {
    ...
    description: {
      pl: "Kompletna platforma SaaS CRM dla polskich biur nieruchomości. 2 plany: Solo (indywidualni agenci) i Enterprise (biura). Integracje z Otodom, NOE 2.0, Domy.pl, Morizon-Gratka. AI matchmaking, RAG chatbot, generator opisów. 23 tabele z RLS, 28+ widoków dashboard, 791 testów. Aplikacja mobilna Capacitor 8.",
    ...
    features: {
      pl: [
        ...
        "217 testów security, 791 testów łącznie",
      ...
      en: [
        ...
        "23 tables with Row Level Security, 28+ dashboard views",
        "217 security tests, 791 total tests",
  ...
  },
  {
    ...
    description: {
      pl: "Ekosystem 10 produktów dla prefabrykacji drewnianej. Viewer 3D (BTLx/WUP/TCN/IFC), Convert (5 post-procesorów CNC), Wall/Roof/Floor (parametryczne projektanty), Calc (Eurocode 5, 20 modułów), CNC Studio, Nest, API. 103 moduły JS, 1909 testów. Vanilla JS + Three.js + Konva.js + Python serverless.",
      en: "Ecosystem of 10 products for timber prefabrication. 3D Viewer (BTLx/WUP/TCN/IFC), Convert (5 CNC post-processors), Wall/Roof/Floor (parametric designers), Calc (Eurocode 5, 20 modules), CNC Studio, Nest, API. 103 JS modules, 1,909 tests. Vanilla JS + Three.js + Konva.js + Python serverless.",
    },
    longDescription: {
    ...
    features: {
      pl: [
        ...
        "103 moduły JavaScript, 26 stron HTML, zero bundlera",
        "1909 testów Python (pytest), 22 klasy testowe",
      ...
      en: [
        ...
        "103 JavaScript modules, 26 HTML pages, zero bundler",
        "1,909 Python tests (pytest), 22 test classes",
  ...
  },
  {
    ...
    description: {
      pl: "Ekosystem 3 aplikacji sportowych. TrainPilot: tracking aktywności, Garmin sync, AI trener, żywienie z rozpoznawaniem zdjęć, PMC/CTL/ATL. TrainMate: swipe discovery partnerów, czat Pusher, Strava OAuth, gym check-ins, forum. Health: monitoring BLE/WHOOP, 20+ algorytmów HRV, recovery/strain/sleep scoring, AI coach.",
      en: "Ecosystem of 3 sport apps. TrainPilot: activity tracking, Garmin sync, AI coach, meal photo recognition, PMC/CTL/ATL. TrainMate: swipe partner discovery, Pusher chat, Strava OAuth, gym check-ins, forum. Health: BLE/WHOOP monitoring, 20+ HRV algorithms, recovery/strain/sleep scoring, AI coach.",
    },
    longDescription: {
      pl: "Athlix to ekosystem 3 komplementarnych aplikacji sportowych:\n\n🏋️ TrainPilot — osobisty trener AI:\n• Garmin Connect sync (aktywności, moc, HRV, TSS)\n• AI trener dzienny (Claude) — briefing z rekomendacjami\n• Żywienie z rozpoznawaniem zdjęć (Claude Vision)\n• PMC: CTL/ATL/TSB/ACWR z optymalizacją batch\n• Google Calendar sync, planowanie treningów\n• Suplementy, dziennik (nastrój/energia), pomiary ciała\n• 265 testów, 64 routes, Capacitor 8 mobilna\n\n🤝 TrainMate — społeczność sportowców:\n• Swipe discovery partnerów (9-czynnikowy algorytm matchowania)\n• Real-time czat Pusher (DM + grupowe sesje treningowe)\n• Strava OAuth2 (import aktywności, sprzęt, best efforts)\n• Moduł siłowni: check-in, logi treningowe, spotter requests, rekordy PR\n• Sesje treningowe (jednorazowe + cykliczne, GPX trasy)\n• Forum z kategoriami, huby sportowe, leaderboard\n• Plany treningowe (twórz/udostępniaj/oceniaj)\n• 35+ tabel w bazie, Google Maps integracja\n\n❤️ Health — alternatywa WHOOP:\n• Monitoring BLE w czasie rzeczywistym (Web Bluetooth API)\n• Reverse-engineered WHOOP v4/v5 (65+ komend BLE)\n• 20+ algorytmów HRV: RMSSD, SDNN, pNN50, LF/HF, DFA Alpha-1, Poincaré, entropia\n• Recovery scoring (0-100, 6 czynników ważonych)\n• Strain scoring (0-21, TRIMP logarytmiczny)\n• Sleep tracking z fazami (deep/REM/light)\n• AI health coach (Claude) — personalizowane insighty\n• VO2max estimation, ACWR, illness warning\n• 121 testów, szyfrowanie AES-256-GCM, PBKDF2",
      en: "Athlix is an ecosystem of 3 complementary sport applications:\n\n🏋️ TrainPilot — personal AI trainer:\n• Garmin Connect sync (activities, power, HRV, TSS)\n• Daily AI coach (Claude) — briefing with recommendations\n• Meal photo recognition (Claude Vision)\n• PMC: CTL/ATL/TSB/ACWR with batch optimization\n• Google Calendar sync, training planning\n• Supplements, journal (mood/energy), body measurements\n• 265 tests, 64 routes, Capacitor 8 mobile\n\n🤝 TrainMate — athlete community:\n• Swipe partner discovery (9-factor match algorithm)\n• Real-time Pusher chat (DM + group training sessions)\n• Strava OAuth2 (activity import, gear, best efforts)\n• Gym module: check-in, workout logs, spotter requests, PR records\n• Training sessions (one-off + recurring, GPX routes)\n• Forum with categories, sport hubs, leaderboard\n• Training plans (create/share/rate)\n• 35+ database tables, Google Maps integration\n\n❤️ Health — WHOOP alternative:\n• Real-time BLE monitoring (Web Bluetooth API)\n• Reverse-engineered WHOOP v4/v5 (65+ BLE commands)\n• 20+ HRV algorithms: RMSSD, SDNN, pNN50, LF/HF, DFA Alpha-1, Poincaré, entropy\n• Recovery scoring (0-100, 6 weighted factors)\n• Strain scoring (0-21, TRIMP logarithmic)\n• Sleep tracking with stages (deep/REM/light)\n• AI health coach (Claude) — personalized insights\n• VO2max estimation, ACWR, illness warning\n• 121 tests, AES-256-GCM encryption, PBKDF2",
    },
    status: "development",
    ...
    features: {
      pl: [
        "3 aplikacje: TrainPilot (trening) + TrainMate (społeczność) + Health (recovery)",
        "TrainPilot — Garmin Connect sync (TSS/NP/IF), AI trener dzienny (Claude)",
        "TrainPilot — żywienie z rozpoznawaniem zdjęć (Claude Vision), planer posiłków",
        "TrainPilot — PMC: CTL/ATL/TSB/ACWR, batch computation zoptymalizowany",
        "TrainMate — swipe discovery partnerów (9-czynnikowy algorytm matchowania)",
        "TrainMate — real-time czat Pusher (DM + sesje grupowe)",
        "TrainMate — Strava OAuth2 (import aktywności, sprzęt, best efforts, badge)",
        "TrainMate — moduł siłowni: check-in, PR tracking, spotter requests",
        "TrainMate — forum z kategoriami, huby sportowe, leaderboard, plany treningowe",
        "Health — monitoring BLE w czasie rzeczywistym (Web Bluetooth API)",
        "Health — reverse-engineered WHOOP v4/v5 (65+ komend BLE protokołu)",
        "Health — 20+ algorytmów HRV (RMSSD, DFA Alpha-1, Poincaré, entropia)",
        "Health — recovery scoring (0-100), strain (0-21 TRIMP), sleep tracking z fazami",
        "Health — AI health coach (Claude), VO2max estimation, illness warning",
        "58+ tabel w bazach danych łącznie, 386+ testów, PWA z offline mode",
      ],
      en: [
        "3 apps: TrainPilot (training) + TrainMate (community) + Health (recovery)",
        "TrainPilot — Garmin Connect sync (TSS/NP/IF), daily AI coach (Claude)",
        "TrainPilot — meal photo recognition (Claude Vision), meal planner",
        "TrainPilot — PMC: CTL/ATL/TSB/ACWR, optimized batch computation",
        "TrainMate — swipe partner discovery (9-factor match algorithm)",
        "TrainMate — real-time Pusher chat (DM + group sessions)",
        "TrainMate — Strava OAuth2 (activity import, gear, best efforts, badge)",
        "TrainMate — gym module: check-in, PR tracking, spotter requests",
        "TrainMate — forum with categories, sport hubs, leaderboard, training plans",
        "Health — real-time BLE monitoring (Web Bluetooth API)",
        "Health — reverse-engineered WHOOP v4/v5 (65+ BLE protocol commands)",
        "Health — 20+ HRV algorithms (RMSSD, DFA Alpha-1, Poincaré, entropy)",
        "Health — recovery scoring (0-100), strain (0-21 TRIMP), sleep staging",
        "Health — AI health coach (Claude), VO2max estimation, illness warning",
        "58+ database tables total, 386+ tests, PWA with offline mode",
      ],
    },
